...
# Função para carregar dados mensais com autenticação
@st.cache_data
def load_data():
    """
...
# Nova função para carregar dados diários com autenticação
@st.cache_data
def load_daily_data():
    """Carrega dados diários da planilha privada usando autenticação"""
    try:
        gc = setup_google_auth()
        if gc is None:
...
def generate_insights(df_daily, period_start, period_end):
    ...
    try:
        ...
        df_period = df_daily[(df_daily['data'] >= period_start) & (df_daily['data'] <= period_end)]
        
        if len(df_period) > 1:
            # Produtos com maior crescimento
            produto_crescimento = df_period.groupby('nome_universal')['faturamento'].sum().nlargest(3)
        ...
        if len(df_period) >= 7:
            sazonalidade = df_period.groupby('dia_semana')['faturamento'].mean()
        ...
        dias_recentes = 7
        data_limite = period_end - timedelta(days=dias_recentes)
        produtos_sem_venda = df_daily[df_daily['data'] >= data_limite]['nome_universal'].unique()
        todos_produtos = df_daily['nome_universal'].unique()
        produtos_inativos = set(todos_produtos) - set(produtos_sem_venda)
        
        if produtos_inativos:
            insights.append(f"⚠️ **Atenção:** {len(produtos_inativos)} produtos sem vendas nos últimos {dias_recentes} dias.")
        
        # Análise de tendência
        if len(df_period) >= 3:
            tendencia_faturamento = df_period.groupby('data')['faturamento'].sum().pct_change().mean()
...
def create_alerts(df_daily, period_start, period_end):
    """Cria alertas baseados na performance dos produtos"""
    alerts = []
    ...
    try:
        ...
        df_current = df_daily[(df_daily['data'] >= period_start) & (df_daily['data'] <= period_end)]
...
# ABA MENSAL (mantendo toda funcionalidade original)
with tab_mensal:
...
with tab_diaria:
    ...
    if not df_daily.empty:
        ...
        if not df_filtered.empty:
            ...
            total_faturamento_diario = df_filtered['faturamento'].sum()
            total_pedidos_diario = df_filtered['quantidade_pedidos'].sum()
            total_unidades_diario = df_filtered['total_unidades'].sum()
            ...
            with col_season1:
                ...
                weekday_performance = df_filtered.groupby('dia_semana')['faturamento'].sum().reindex([
            ...
            with col_season2:
                ...
                if len(df_filtered) > 0:
                    ...
                    top_products_list = df_filtered.groupby('nome_universal')['faturamento'].sum().nlargest(10).index
            ...
            if not df_previous.empty:
                ...
                current_total = df_filtered['faturamento'].sum()
                ...
                current_orders = df_filtered['quantidade_pedidos'].sum()
            ...
            with col_alerts:
                ...
                alerts = create_alerts(df_daily, period_start, period_end)
            ...
            st.download_button(
                ...
                data=csv_data,
                file_name=f"dados_diarios_{period_start.strftime('%Y%m%d')}_{period_end.strftime('%Y%m%d')}.csv",
                mime="text/csv"
            )
